# PC
# 宗旨：根据不同的Args生成不同的prompt，不同函数针对的任务不同
def get_action_prompt(instruction, clickable_infos, width, height,
                      thought_history, summary_history, action_history,
                      last_summary, last_action, reflection_thought, add_info,
                      error_flag, completed_content, memory, use_som,
                      icon_caption, location_info):
    prompt = "### Background ###\n"
    if use_som == 1:
...
def get_reflect_prompt(instruction, clickable_infos1, clickable_infos2, width,
                       height, summary, action, add_info):
